...
import { Text, Button, useMatchBreakpoints } from 'packages/uikit'
...
const MigrationSticky: React.FC<MigrationStickyProps> = ({ step, handleClick }) => {
  ...
  const title: string = isStep1 ? t('Unstaking LP Tokens') : t('Stake in the new contract.')
  const subTitle: string = isStep1
    ? t('All the earned VEN will be harvested to your wallet upon unstake.')
  ...
  if (!account) {
    return (
      <Container>
        <TextGroup>
          <TextTitle bold>{t('Migration')}</TextTitle>
