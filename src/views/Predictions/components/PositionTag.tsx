...
import { ArrowUpIcon, ArrowDownIcon, Flex, FlexProps, Text, useTooltip, TooltipText, InfoIcon } from 'packages/uikit'
...
const PositionTag: React.FC<PositionTagProps> = ({ betPosition, children, ...props }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      <Text>
        {t(
          'The Locked Price & Closed Price are exactly the same (within 8 decimals), so neither side wins. All funds entered into UP and DOWN positions will go to the weekly burn.',
        )}
      </Text>
    </>,
    { placement: 'right' },
