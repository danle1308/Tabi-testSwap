...
import { Text, Flex, Box, Skeleton, TooltipText, useTooltip } from 'packages/uikit'
...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of TABI up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max TABI entry is capped by minimum between your average TABI balance in the IFO TABI pool, or the pool’s hard cap. To increase the max entry, Stake more TABI into the IFO TABI pool',
        )

  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max TABI entry is capped by your average TABI balance in the IFO TABI pool. To increase the max entry, Stake more TABI into the IFO TABI pool',
        )
      : t(
          'For the unlimited sale, Max TABI entry is capped by your average TABI balance in the IFO TABI pool. To increase the max entry, Stake more TABI into the IFO TABI pool',
        )

  const tooltipContent = poolId === PoolIds.poolBasic ? basicTooltipContent : unlimitedToolipContent

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. TABI entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('TABI to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
