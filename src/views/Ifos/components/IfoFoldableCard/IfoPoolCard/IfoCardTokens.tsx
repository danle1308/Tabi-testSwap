...
import { VENVlxLpToken } from 'config/constants/ifo'
...
const IfoCardTokens: React.FC<IfoCardTokensProps> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough VEN to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your VEN.',
    ),
    { placement: 'bottom' },
  )
          ...
          <Box>
            <MessageText display="inline">
              {publicIfoData.status === 'finished'
                ? t('Activate TabiSwap Profile to take part in next IFO‘s!')
                : t('You need an active TabiSwap Profile to take part in an IFO!')}
            </MessageText>{' '}
    ...
    if (ifo.version === 3 || (ifo.version === 3.1 && poolId === PoolIds.poolUnlimited)) {
      message = (
        <Message my="24px" p="8px" variant="danger">
          <Box>
            <MessageText display="inline">
              {t('You don’t have any average VEN balance available to commit in the IFO VEN pool.')}
            </MessageText>{' '}
          ...
          {ifov31Msg || (
            <>
              <Text textAlign="center" fontSize="14px">
                {t('To participate in the next IFO, stake some VEN in the IFO VEN pool!')}
              </Text>
