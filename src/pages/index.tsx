import { FACTORY_ADDRESS } from '@tabi-dex/sdk'
  ...
  const usersQuery = gql`
    query userCount($since: ISO8601DateTime, $till: ISO8601DateTime) {
      ethereum(network: bsc) {
        dexTrades(exchangeName: { in: ["TabiSwap", "TabiSwap v2"] }, date: { since: $since, till: $till }) {
          count(uniq: senders)
        }
      }
    }
  ...
  try {
    const result = await infoServerClient.request(gql`
      query tvl {
        ...
        token(id: "0xabf26902fd7b624e0db40d31171ea9dddf078351") {
          derivedUSD
        }
      }
    `)
