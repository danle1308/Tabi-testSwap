...
export const getTokenPricesFromFarm = (farms: SerializedFarm[]) => {
  return farms.reduce((prices, farm) => {
    ...
    if (
      !(
        /* The `prices` object is used to store the prices of different tokens in a farm. It is
    initialized as an empty object. The function `getTokenPricesFromFarm` iterates over an
    array of farms and for each farm, it checks if the quote token address and token address
    are already present in the `prices` object. If not, it adds the token price in USD to the
    `prices` object using the token address as the key. The `prices` object is then returned
    with the token prices for all the farms. */
        prices[quoteTokenAddress]
      )
    ) {
