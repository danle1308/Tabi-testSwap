...
export const fetchPoolsStakingLimits = async (
  ...
  const poolStakingResultRaw = await multicallv2(sousChefABI, poolStakingCalls, { requireSuccess: false })
  ...
  return poolStakingChunkedResultRaw.reduce((accum, stakingLimitRaw, index) => {
    ...
    const numberBlocksForUserLimit = 0
    return {
      ...accum,
...
export const fetchPoolsProfileRequirement = async (): Promise<{
  ...
  const poolProfileRequireResultRaw = await multicallv2(sousChefABI, poolProfileRequireCalls, { requireSuccess: false })
