...
const getFarmQuoteTokenPrice = (
  ...
  if (farm.quoteToken.symbol === 'WMON') {
    return bnbPriceBusd
  }
  ...
  if (quoteTokenFarm.quoteToken.symbol === 'WMON') {
...
const getFarmsPrices = async (farms: SerializedFarm[]) => {
  const bnbBusdFarm = farms.find((farm) => farm.token.symbol === 'USDC' && farm.quoteToken.symbol === 'WMON')
  const bnbPriceBusd = bnbBusdFarm.tokenPriceVsQuote ? BIG_ONE.div(bnbBusdFarm.tokenPriceVsQuote) : BIG_ZERO
  // const res = await fetch(
  //   `https://api.etherscan.io/api?module=stats&action=ethprice&apikey=${process.env.NEXT_SCANAPI_KEY}`,
  // )
  ...
  const farmsWithPrices = farms.map((farm) => {
    ...
    // const data = {
    //   // tokenPriceBusd: new BigNumber(farm.tokenPriceVsQuote).times(ethusd).toJSON(),
    //   tokenPriceBusd: tokenPriceBusd.toJSON(),
    //   // quoteTokenPriceBusd: ethusd,
