...
function getCoreFarms(farms: { pid: number }[]): number[] {
  // Extract the existing pid values, excluding pid 0
  const existingPids = farms.map((item) => item.pid).filter((pid) => pid !== 0)
 ...
 * 1 = MTS-ETH LP
 */
const coreFarmPIDs = CHAIN_ID === String(ChainId.MAINNET) ? getCoreFarms(farmsConfig) : []
